...
namespace TapTap.AntiAddiction
{
    public sealed class VietnamAntiAddictionWorker : BaseAntiAddictionWorker
    {
        ...
        protected override async Task VerifyManuallyAsync(bool activelyVerifyManually)
        {
            ...
            do 
            {
                try 
                {
                    ...
                    UIManager.Instance.OpenToast(Config.Current.UIConfig.InputRealNameInfoVietnam.submitSuccessMsg, UIManager.GeneralToastLevel.Success);
        ...
        public override Task<bool> IsStandaloneEnabled()
        {
        ...
        protected override async Task<int> InternalStartup(string userId, bool useTapLogin){
            // 1. 如果没验证过,需要弹出验证窗口,并完成验证----------------------
            try {
                await VerifyManuallyAsync(true);
            }
            catch  (TaskCanceledException) {
        ...
        public override async Task<PlayableResult> CheckPlayableOnPollingAsync()
        {
